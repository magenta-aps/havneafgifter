...
from havneafgifter.models import DisembarkmentSite, Municipality
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        for site in DisembarkmentSite.objects.filter(prisme_code__isnull=True):
            placename = site.name.lower()
            try:
                site.prisme_code = self.localities[site.municipality][placename]
                site.save()
            except KeyError:
                print(
                    f"Could not find locality for site "
                    f"{site.pk} ({site.name}, {Municipality(site.municipality).name})"
                )
            else:
                print(
                    f"Found locality for site {site.pk} "
                    f"directly by string match: "
                    f"({site.name}, {Municipality(site.municipality).name})"
                )

        print(
            f"There are now "
            f"{DisembarkmentSite.objects.filter(prisme_code__isnull=True).count()} "
            f"disembarkment sites without locality set."
        )
